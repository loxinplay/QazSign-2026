...
function startPythonServer(): Promise<void> {
  return new Promise((resolve, reject) => {
    const pythonPath = path.resolve(process.cwd(), "python_server", "app.py");
    
    log("Starting Python AI server...");
    
    pythonProcess = spawn("python", ["-u", pythonPath], {
      cwd: path.resolve(process.cwd(), "python_server"),
    ...
    pythonProcess.stdout?.on("data", (data) => {
      ...
      if (message) log(`[Python] ${message}`);
      if (message.includes("Models loaded successfully")) {
        log("Python AI server ready!");
